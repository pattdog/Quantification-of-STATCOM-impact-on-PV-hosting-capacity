...
function solve_and_report(data_math, label; sbase_kva=SBASE_KVA, vbase_kv=VBASE_KV)
    ...
    if string(status) in ["LOCALLY_SOLVED", "OPTIMAL"]
        ...
        if !isempty(pv_gens) && haskey(sol, "gen")
            ...
            # NOTE: capacity is now g["p_rating"] (true kW nameplate), NOT
            # g["pmax"] — pmax is a loose s_pu ceiling since the fixed
            # generator bounds change, and would overstate capacity here.
            pg_rated    = [get(g, "p_rating", sum(g["pmax"])) for (i, g) in pv_gens]
...
if do_case2
    ...
    for pv_kw in PV_KW_LEVELS
        ...
        pv_ids_upf = add_pv!(dm_upf; pv_kw=pv_kw, s_scale=1.25, spacing=1,
        ...
        add_pv!(dm_free; pv_kw=pv_kw, s_scale=1.25, spacing=1,
                pv_cost=PV_COST, enable_vvc=false)
        ...
        add_pv!(dm_vvc; pv_kw=pv_kw, s_scale=1.25, spacing=1,
