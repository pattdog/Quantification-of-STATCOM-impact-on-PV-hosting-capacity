#=
==============================================================================
constraints_PBalance.jl -- WITH STATCOM inter-phase active power exchange extension
==============================================================================
CHANGES vs. the original (OG) file, marked with PATCH START / PATCH END:

  1. Generator loop, WYE branch: after the existing pg/qg-from-crg/cig
     equality constraints, STATCOM generators (identified via
     generator["statcom_p_exchange"] == true) get two new constraints
     instead of / in addition to the plain box bounds:

        (a) Per-phase joint capability:  pg[p]^2 + qg[p]^2 <= s_rated[p]^2
        (b) DC-bus coupling:             sum(pg[phases,id]) == p_loss

     This replaces the old assumption pmin=pmax=0 (STATCOM as pure Q source)
     with the physically correct constraint for a 3-phase VSC sharing a
     common DC bus: total AC-side active power across all three phases must
     sum to zero (or to converter losses), while each phase individually
     is free to import or export P, bounded by the shared S rating.

     Validated in isolation (free pg/qg) and embedded in a minimal unbalanced
     network (pg/qg derived from vr/vi/crg/cig) via:
        test_statcom_p_exchange.jl        (Tier 1)
        test_statcom_tier2_network.jl     (Tier 2)

  2. Non-STATCOM generators (PV, etc.) are COMPLETELY UNCHANGED -- they still
     go through the original box-bound path exactly as before. This patch
     only activates for generators explicitly flagged as
     statcom_p_exchange = true in their data dict (set this in your
     add_statcom!-equivalent script).

REQUIRED generator data fields for a STATCOM using this new path:
    generator["statcom_p_exchange"] = true
    generator["s_rated"]            = [S1, S2, S3]   # per-phase MVA/pu rating
                                                        (or a scalar, broadcast)
    generator["p_loss"]             = 0.0             # optional, defaults to 0
                                                        # (set >0 if you want to
                                                        # model converter losses
                                                        # as a small net P draw)

Everything else in this file (bus loop, branch loop, load loop, bus KCL loop,
DELTA-configuration generator branch) is IDENTICAL to the original.
==============================================================================
=#
...
for (i, bus) in ref[:bus]
    ...
    if i in keys(ref[:ref_buses])
        ...
        vrefre = real.(vref)
        vrefim = imag.(vref)
        # FIX (pre-existing bug, unrelated to STATCOM patch): vr[:,i]/vi[:,i] are
        # always length n_ph=4 (zero-padded), but vrefre/vrefim are only as long
        # as this bus's actual terminal set (e.g. 3 for a 3-wire bus). Index by
        # bus["terminals"] so both sides match in length.
...
for (id, generator) in ref[:gen]
    ...
    pmin = get(generator, "pmin", fill(-Inf, N));  pmin = isempty(size(pmin)) ? [pmin] : pmin
    ...
    # =========================================================================
    # PATCH START: STATCOM inter-phase P-exchange flag/data
    # =========================================================================
    ...
    if configuration==PMD.WYE || length(pmin)==1 || nphases==1
        ...
        if is_statcom_p_exchange
            # =====================================================================
            # PATCH START: STATCOM path -- loose safety box only.
            # The real limiting constraints are the S^2 capability and DC coupling
            ...
            # --- ORIGINAL box-bound path, unchanged, for all non-STATCOM-P-exchange gens ---
            if !isempty(nonInf_pmin_pmax_idx)
        ...
        # =========================================================================
        # PATCH START: STATCOM inter-phase P-exchange -- the new physics
        #
        # Placement rationale (same logic as the earlier VVC patch):
        #   - Must come AFTER the pg/qg equality constraints just above, since
        #     these constraints reference pg[...]/qg[...] as already-linked
        #     expressions (via crg/cig), not free variables.
        #   - Must come BEFORE crg_bus/cig_bus are finalized after the full
        #     `for (id, generator) in ref[:gen]` loop, i.e. still inside the loop.
        #   - Does NOT touch the loose box-bound block above -- that block still
        #     runs (using generator["pmin"]/["pmax"], which should be set to
        #     +-s_rated or looser in your add_statcom!-equivalent script), but it
        #     becomes a non-binding outer safety box once these constraints below
        #     are the real shaping constraints.
        # =========================================================================
        if is_statcom_p_exchange
            ...
            # NORMALIZED FORM: mathematically identical feasible region to
            # pg^2 + qg^2 <= s_rated^2, but scaled so the constraint itself
            # is always O(1) regardless of s_rated's absolute pu magnitude.
            # Needed because with sbase=100,000 kVA and a real device rated
            # e.g. 5 kVAr, s_rated_pu ~ 5e-5, so the UNNORMALIZED constraint's
            # values sit around 1e-9 -- five orders of magnitude below
            # Ipopt's default constr_viol_tol (~1e-4), so it was silently
            # not being enforced (confirmed: solved points showed S up to
            # 41% over rating, LOCALLY_SOLVED status notwithstanding).
            # ANONYMOUS constraint (no name before the index set) -- with more
            # than one P-exchange STATCOM in the model, a NAMED container here
            # (e.g. statcom_cap[...]) throws "already attached to this model"
            # on the second generator, since JuMP won't silently re-register a
            # shared symbol across loop iterations. Anonymous avoids this
            # entirely -- same constraint, no shared name.
            JuMP.@constraint(model, [p in phases],
            (pg[p,id]/s_rated[findfirst(==(p), phases)])^2
          + (qg[p,id]/s_rated[findfirst(==(p), phases)])^2 <= 1)

            # (b) DC-bus coupling: sum of active power across all three phases
            #     must equal zero (lossless) or p_loss (if modelling losses)
            #     Also anonymous, same reasoning as above.
